...
routine_tempo
    MOVLW 0x21 ; 25 en décimal
    
    MOVWF var0 ; var0*var1*var2 = 25*100^2 = 2,5*10^5
	       ; *4 cycles/instruction, on obtient 10^6ms = 1s de délai
    
    MOVLW 0x64 ; 100 en décimal
    
    loop0
	MOVWF var1 ; 3 * 10^2 instructions
	loop1
	    MOVWF var2 ; 3 * 10^4 instructions
	    loop2
		DECF var2, 1 ; 2 * 2,5 * 10^5 = 5*10^5 instructions
		BNZ loop2 ; 4 cycles par instruction, fréquence de 4MHz
			; 2,5*10^-7s par cycle => 10^-6s par instruction
	    DECF var1, 1
	    BNZ loop1
	DECF var0, 1
	BNZ loop0
...
loop
    ...
    MOVWF PORTA ; Ecriture des bits sur les sorties
    MOVWF PORTC ; PORTA => 4 premières LEDs, PORTC => 4 dernières LEDs
